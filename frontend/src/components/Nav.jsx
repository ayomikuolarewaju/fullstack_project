  ...
  return (
    <div className="flex items-center justify-around mx-auto p-4 text-white w-full h-[100px]">
      <div className="flex items-center justify-between w-[500px]">
        <div className="font-san text-2xl font-bold text-red-800">
          <Link href="/">
            <h1 className="font-dance text-3xl font-bold">cinematch</h1>
          </Link>
        </div>
        <div className="flex items-center justify-between w-[300px] font-semibold text-black no-underline cursor-pointer">
          {menus.map((menu, index) => (
            <div key={index}>
              <Link href={menu.go} className="cursor-pointer">
                {menu.title}
              </Link>
            </div>
          ))}
